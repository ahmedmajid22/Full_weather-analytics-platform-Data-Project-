{{
    config(
        materialized='table',
        indexes=[{'columns': ['city_id', 'reading_at'], 'unique': True}]
    )
}}
...
WITH rolling_stats AS (
    SELECT
        ...
        PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY temperature_c) AS p25,
        PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY temperature_c) AS p75,
...
scored AS (
    SELECT
        ...
        s.stddev_temp,
        s.sample_size,
        ROUND((r.temperature_c - s.mean_temp) / NULLIF(s.stddev_temp, 0), 3)          AS z_score,
        ROUND((r.temperature_c - s.mean_temp) / NULLIF(s.p75 - s.p25, 0), 3)          AS robust_z_score,
        CASE
...
SELECT * FROM scored ORDER BY ABS(z_score) DESC NULLS LAST